...
logger = Logger("archivedotcom").get()
...
def is_valid_archive_url(url, date):
    ...
    try:
        ...
        logger.info("Failed to open URL [{}]: [{}]".format(archive_url, e))
        return False

    html_doc = response.read()

    if ERROR_404 in html_doc:
        logger.info("Archive URL [{}] is not valid as it contains 404: [{}]".format(archive_url, html_doc))
        return False
...
def find_archive_url(url):
    ...
    logger.info("Searching for latest archived page for URL [{}]".format(url))
    response = urllib2.urlopen(archive_url)
